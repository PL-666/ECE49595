...
public class Stack : MonoBehaviour
{
    ...
    private ObjectType curObjectType;
    ...
    void Start()
    {
        ...
        repulsion = 1.0f;
        saveableObject = gameObject.GetComponent<SaveableObject>();
        curObjectType = saveableObject.GetObjectType();
    ...
    private void OnTriggerEnter(Collider other)
    {
           ...
           gameObject.CompareTag("stack") && other.gameObject.CompareTag("card")))
        {
            if (gameObject.GetComponent<Drag>().selected == true)
            {
                if (stackableList.Contains(other.GetComponent<SaveableObject>().GetObjectType().ToString()))
                {
                    _Stack(other);
